...
public class Pathfinding
{
    ...
    private const int MOVE_DIAGONAL_COST = 14; //pythagorova věta 
    ...
    {
        grid = new Grid(width, height, 1, new Vector3(-8.5f, -5f), this);
        grid.SetUpNeighbors();
    }
    ...
    public List<Vector2> FindPath(Vector2 startWorldPosition, Vector2 endWorldPosition)
    {
        ...
        { 
            ...
            {
                vectorPath.Add(new Vector2(pathNode.x, pathNode.y) * grid.cellsize + Vector2.one * grid.cellsize / 2 + (Vector2)grid.originPosition);
            }
            return vectorPath; 
        }
    }
    ...
    public List<PathNode> FindPath(int startX, int startY, int endX, int endY)
    {
        ...
        while (openList.Count > 0)
        {
            ...
            foreach (PathNode neighbour in currentNode.neighbourList)
            {
    ...
    private PathNode GetNode(int x, int y) => grid.GetGridNode(x, y);
    ...
    private List<PathNode> CalculatePath(PathNode endNode)
    {
        ...
        path.Reverse(); //from end to beginning
        return path; 
    }

    private int CalculateDistanceCost(PathNode a, PathNode b)
    {
        //Debug.Log(a.x + " " + a.x + "  " + b.x + " " + b.y);
